...
export default function Create() {
    const { data, setData, post, processing, errors } = useForm({
        ...
        cells: Array.from({ length: 25 }).map((_, i) => ({
            row_index: Math.floor(i / 5),
            col_index: i % 5,
            label: Math.floor(i / 5) === 2 && i % 5 === 2 ? 'FREE SPACE' : '',
            risk_weight: Math.floor(i / 5) === 2 && i % 5 === 2 ? 0 : 1,
        })),
        score_interpretations: [] as { min_score: number; max_score: number; label: string }[],
    });
    ...
    // const updateCellWeight = (row: number, col: number, weight: number) => {
    //     const newCells = [...data.cells];
    //     const index = newCells.findIndex((c) => c.row === row && c.col === col);

    //     if (index !== -1) {
    //         newCells[index].risk_weight = weight;
    //         setData('cells', newCells);
    //     }
    // };
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(admin.cards.store().url);
    };

    return (
        <>
            <Head title="Create Bingo Arena" />
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h2 className="text-5xl font-black tracking-tighter text-foreground uppercase italic">
                            Arena <span className="text-primary">Designer</span>
                        </h2>
                        <p className="text-lg font-medium text-muted-foreground">
                            Configure the grid and mission parameters.
                        </p>
                    </div>
                </div>
                ...
                <form onSubmit={submit} className="grid gap-10 lg:grid-cols-2">
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="flex gap-4">
                            ...
                            >
                                {/* <Link href={admin.cards.activate().url}>Abort</Link> */}
                                Abort
                            </Button>
                        </div>
                    </div>
                                        ...
                                        <div
                                            ...
                                            className={`relative flex aspect-square flex-col items-center justify-center rounded-xl border-2 p-1 transition-all ${
                                                isFree
                                                    ...
                                                    : 'border-border/50 bg-background/50 focus-within:border-primary hover:border-primary/30 dark:border-white/10'
                                            }`}
                                        >
                                            {isFree ? (
